...
class StockData:
    ...
    def _download_data(self):
        for ticker in self.tickers:
            yf_tk = ticker + '.sa'
            self._complete_data[ticker] = yf.download(tickers=yf_tk,start=date(2010,1,1),end=self.end_date,progress=False)
...
def backtest_markowitz(tickers, ndays, plot=False):
    #Ainda não tem troca de ativos no meio
    data = StockData(tickers)
    dataframe = data.get_column_data('Adj Close').dropna()
    dataframe_open = data.get_column_data('Open').dropna()
    df=dataframe[ndays:]
    #df_open = dataframe_open[ndays:]
    start_time = time.time()
    pesos_aleat = []
    ...
    for i in range(len(ret_close)):
        ...
        #close = StockData(tickers,end_date=dt_recalc[i]).get_column_data('Adj Close') demora 28x mais tempo
        mark = Markowitz(close)
        pesos_aleat.append(mark.fit(sg=False))
    pesos_aleat = pd.DataFrame(pesos_aleat)
    avg_close = np.array(pesos_aleat)*np.array(ret_close)
...
dados = StockData(tickers)

close = dados.get_column_data('Adj Close')
